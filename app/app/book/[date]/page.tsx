...
import { ArrowLeft, ArrowRight, CalendarDays, PenLine } from "lucide-react";
...
export default function BookEntryPage() {
  ...
  useEffect(() => {
    ...
    async function loadEntry() {
      const { data, error: queryError } = await supabase.from("diary_entries").select("content").eq("entry_date", date).eq("user_id", USER_ID).maybeSingle();
      if (!active) return;
  ...
  return (
    <main className="book-stage flex-1 px-4 py-8 sm:px-8 sm:py-12">
      <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-3 pb-6">
        ...
        <button className="button button-primary" onClick={() => router.push(`/day/${date}`)}><PenLine size={16} /> Edit page</button>
      </div>
