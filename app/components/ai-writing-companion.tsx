...
import { Bot, Check, Copy, ListChecks, LoaderCircle, MessageCircle, Sparkles, X } from "lucide-react";
import type { Editor } from "@tiptap/react";
import { diaryTemplateMap } from "@/lib/templates";

type Action = "start" | "guided" | "diary-entry" | "expand" | "improve" | "organize" | "create-template" | "convert-template" | "recommend-template" | "extract-tasks";
...
export default function AIWritingCompanion({ editor, onSaveTemplate, onGuidedWriting }: Props) {
  ...
  const runAction = async (nextAction: Action, requestInstruction = instruction) => {
    if (!editor) return;
    setAction(nextAction); setLoading(true); setError(""); setResult(""); setApprovedTasks([]);
    ...
    const response = await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: nextAction, text, instruction: requestInstruction }) });
  ...
  const accept = () => {
    ...
    if (action === "extract-tasks") {
      try { const tasks = JSON.parse(result.replace(/^```json\s*|\s*```$/g, "")) as { task: string }[]; setApprovedTasks(tasks.map((item) => item.task)); } catch { setError("The task preview was not valid. Try again."); }
      return;
    }
